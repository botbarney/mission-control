...
export default function MissionControl() {
  ...
  return (
    <div className="min-h-screen bg-gray-950 text-gray-100 font-sans">
      ...
      <main className="max-w-7xl mx-auto px-4 py-6 space-y-6">
        ...
        <div className="flex flex-wrap gap-3">
          <button
            ...
            className="px-4 py-2 bg-cyan-600 hover:bg-cyan-500 rounded-lg font-medium transition-colors flex items-center gap-2"
          >
          ...
          <div className="bg-gray-900/50 rounded-xl border border-gray-800 p-5">
            <h2 className="text-lg font-semibold mb-4 flex items-center gap-2">
              ...
              <span className="text-xs bg-gray-800 px-2 py-0.5 rounded-full text-gray-400">
                {agents.filter(a => a.status === 'active' || a.status === 'working').length} active
              </span>
            </h2>
        ...
        <div className="bg-gray-900/50 rounded-xl border border-gray-800 p-5">
          <h2 className="text-lg font-semibold mb-4 flex items-center gap-2">
            ...
            <span className="text-xs bg-gray-800 px-2 py-0.5 rounded-full text-gray-400 animate-pulse">
              LIVE
            </span>
          </h2>
        ...
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-gray-900 rounded-xl border border-gray-700 p-6 w-full max-w-md">
            ...
            <div className="space-y-2">
              {['iOS Developer', 'ASO Specialist', 'Research Agent', 'Growth Specialist'].map((type) => (
                <button
                  key={type}
                  className="w-full text-left p-3 bg-gray-800 hover:bg-gray-700 rounded-lg border border-gray-700 hover:border-cyan-500 transition-colors"
                >
                  {type}
                </button>
              ))}
      ...
      {/* Mobile indicator */}
      <div className="sm:hidden fixed bottom-4 right-4 bg-cyan-600 text-white px-3 py-1 rounded-full text-xs">
        Mobile View
      </div>
    </div>
  );
